...
namespace FTD2XX_NET
{
    ...
    public class FTDI
    {
        ...
        private void FindFunctionPointers()
        {
            ...
            pFT_SetDivisor = GetProcAddress(hFTD2XXDLL, "FT_SetDivisor");
        }
        ...
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        private delegate FT_STATUS tFT_SetDivisor(IntPtr ftHandle, UInt16 usDivisor);
        ...
        IntPtr pFT_SetDivisor = IntPtr.Zero;
        #endregion
        ...
        public FT_STATUS SetDivisor(UInt16 divisor)
        {
            // Initialise ftStatus to something other than FT_OK
            FT_STATUS ftStatus = FT_STATUS.FT_OTHER_ERROR;
            ...
            // Check for our required function pointers being set up
            if (pFT_SetDivisor != IntPtr.Zero)
            {
                tFT_SetDivisor FT_SetDivisor = (tFT_SetDivisor)Marshal.GetDelegateForFunctionPointer(pFT_SetDivisor, typeof(tFT_SetDivisor));

                if (ftHandle != IntPtr.Zero)
                {
                    // Call FT_SetBaudRate
                    ftStatus = FT_SetDivisor(ftHandle, divisor);
                }
            }
            else
            {
...
#if DEBUG
                    MessageBox.Show("Failed to load function FT_SetDivisor.");
#endif
                }
            }
            return ftStatus;
